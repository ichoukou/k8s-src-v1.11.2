select  'prompt " All Data DB Owne r is&2,  Adding Data DB Owner is  &1, Compared table is ' ||atcol.TABLE_NAME|| '";',
       to_char('SELECT COUNT(*) FROM  ( ' || 'SELECT COUNT(*) COUNT FROM ' || '&1' || '.' ||
               atcol.TABLE_NAME || ' WHERE  (' ||
               replace(wm_concat(atcol.COLUMN_NAME), ',', '||') ||
               ' ) IN ( SELECT ' ||
               replace(wm_concat(atcol.COLUMN_NAME), ',', '||') || ' FROM ' || '&2' || '.' ||
               atcol.TABLE_NAME || ' ))  SANT_COUNT ,' ||
               '(SELECT COUNT(*) COUNT FROM ' || '&1' || '.' ||
               atcol.TABLE_NAME || ') COMP_COUNT ' ||
               'WHERE SANT_COUNT.COUNT<>COMP_COUNT.COUNT ;') QUERY_SQL
