...
LOCAL_IMAGE_URL=${3:-"10.0.0.13:5000/abc"}
...
for image in $images
do
   echo "下载镜像为:$image , 镜像地址为：$REMOTE_IMAGE_URL"
   
   docker pull $REMOTE_IMAGE_URL$image >/tmp/error 2>&1 #使用 " 2>&1" 把标准错误 stderr 重定向到标准输出
done
